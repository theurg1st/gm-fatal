...
void lua::executor()
{
	...
	for (auto i : globals::waiting_to_be_executed)
	{
		try
		{
			lua->run_string("", "", i.c_str());
			globals::waiting_to_be_executed.erase(std::remove(globals::waiting_to_be_executed.begin(), globals::waiting_to_be_executed.end(), i.c_str()), globals::waiting_to_be_executed.end());
		}
		catch (...)
		{
		}
