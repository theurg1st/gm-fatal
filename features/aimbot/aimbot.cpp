...
target_info aimbot::find_best_target(c_user_cmd* cmd, c_base_entity* local_player)
{
	...
	for (size_t i = 1; i <= interfaces::entity_list->get_highest_entity_index(); i++)
	{
		...
		if (settings::visuals::esp::players::friend_ignore_aim && !pl_name.empty() && settings::visuals::esp::players::friend_list.contains(pl_name) && settings::visuals::esp::players::friend_list[pl_name].get<bool>())
			continue;
