...
ImVec4 lua_utilities::get_rp_jobs_col(int team)
{
	ImVec4 def_col = { 1.f,1.f,1.f,1.f };
	c_lua_interface* lua = interfaces::lua_shared->get_interface(lua_type_client);
	if (!lua)
		return def_col;
	...
	lua->push_string("r");
	lua->get_table(-2);
	float r = lua->get_number(-1);
	lua->pop();

	lua->push_string("g");
	lua->get_table(-2);
	float g = lua->get_number(-1);
	lua->pop();

	lua->push_string("b");
	lua->get_table(-2);
	float b = lua->get_number(-1);
	lua->pop();
...
ImVec4 lua_utilities::get_team_col(c_base_entity* entity)
{
	ImVec4 def_col = {1.f,1.f,1.f,1.f};
	c_lua_interface* lua = interfaces::lua_shared->get_interface(lua_type_client);
	if (!lua)
	...
	lua->get_field(-1, xorstr("GetColor"));
	if (!lua->is_type(-1, object_type_t::function))
	{
		lua->pop(3);
		return def_col;
...
int lua_utilities::get_armor(c_base_entity* entity)
{
	c_lua_interface* lua = interfaces::lua_shared->get_interface(lua_type_client);
	if (!lua)
		return 0;
